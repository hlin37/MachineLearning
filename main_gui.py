...
class TournamentApp:
    def __init__(self, root, teamGenerator, tournamentGenerator, tournamentDirector):
        ...
        self.current_month = 1  # Start with November
    ...
    def confirm_team(self):
        ...
        if selected_team_name:  # Ensure a team has been selected
            self.selected_team = selected_team_name
            tournamentDirector.remove_from_team_list(self.selected_team)
            
            ## Here it selects which tournaments each team will be going to
            tournamentDirector.decide_which_tournament()

            messagebox.showinfo("Team Selected", f"You have selected: {self.selected_team}")
    ...
    def display_calendar(self):
        ...
        self.select_tournament_information = tk.Label(self.calendar_window, text="", anchor="nw", justify="left", 
                                     background="white", relief="solid", width=30, height=15, padx=10, pady=10, fg="red")
                ...
                if day == 0:
                    ...
                    if day in tournament_dict:
                        for tournament in tournament_dict[day]:
                            tournament_button = tk.Button(day_frame, text=tournament.name,
    ...
    def confirm_tournaments(self):
        if not self.selected_tournaments:
            ...
            tournament_list = ', '.join(self.selected_tournaments.values())
            messagebox.showinfo("Tournaments Confirmed", f"Team: {self.selected_team}\nTournaments: {tournament_list}")
